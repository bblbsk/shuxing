package com.shuxing.lucene.demo;
...
public class IndexandSearch {
	...
	public static void testDemo() throws Exception {
		...
		writer.addDocument(createDocument("中央政治局研究2017年经济工作",
				"中共中央政治局12月9日召开会议，分析研究2017年经济工作，审议通过《关于加强国家安全工作的意见》。中共中央总书记习近平主持会议。"));
		writer.addDocument(createDocument("中央政治局研究2017年经济工作new",
				"test中共中央政治局12月9日召开会议，分析研究2017年经济工作，审议通过《关于加强国家安全工作的意见》。中共中央总书记习近平主持会议。1111111"));
		writer.commit();
		writer.close();
		IndexSearcher searcher = new IndexSearcher(DirectoryReader.open(idx));
		System.out.println("命中个数:"
				+ searcher.search(
						new QueryParser("content", analyzer).parse("content:'经济'"),
						10).totalHits);
		TopDocs topdoc = searcher.search(
				new QueryParser("content", analyzer).parse("content:'经济'"), 10);
