...
public class SMS4 {
    ...
    /**
      ...
      * @Title: decodeTest1
      * @Description: 测试1
      * key 2323236547896542  → 32333233323336353437383936353432 
      * 金额 9334.20 → 393333342E323020 → 8B5D20EF5848830CE449B9D8CB3EF62E
      * @return void    返回类型
      * @throws
      */
    public static void decodeTest1() {
    	byte[] in = {0x39,0x33,0x33,0x34,0x2E,0x32,0x30};
//    	byte[] in = {0x39,0x33,0x33,0x34,0x2E,0x32,0x30,0x20,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08};
    	byte[] key = {0x32,0x33,0x32,0x33,0x32,0x33,0x36,0x35,0x34,0x37,0x38,0x39,0x36,0x35,0x34,0x32};
    	byte[] encode = SMS4.encode16(in, key);
    	byte[] out = {(byte) 0x8B,0x5D,0x20,(byte) 0xEF,0x58,0x48,(byte) 0x83,0x0C,(byte) 0xE4,0x49,(byte) 0xB9,(byte) 0xD8,(byte) 0xCB,0x3E,(byte) 0xF6,0x2E};
//    	String deString = SMS4.decodeSMS4toString(encode, key);
    	String deString = SMS4.decodeSMS4toString(out, key);
...
//    	byte[] in = {0x39,0x33,0x33,0x34,0x2E,0x32,0x30};
    	byte[] key = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x38,0x37,0x36,0x35,0x34,0x33,0x32,0x31};
//    	byte[] encode = SMS4.encode16(in, key);
//    	
    	byte[] out = {0x10,(byte) 0xEF,(byte) 0xF7,(byte) 0xD6,0x3D,(byte) 0x9E,0x64,0xB};
    	byte[] decode = SMS4.decode16(out, key);
    	String deString = new BASE64Encoder().encode(decode);
    	System.out.println(deString);
	}
    ...
    /**
      ...
      * @throws
      */
    public static void decodeTest3(String outString ) {
    	byte[] out = Util.hexStringToBytes(outString);
//    	byte[] out = Util.hexStringToBytes("3E3C86BB27C7DE30342E700CA04105CB");
    	byte[] key = Util.hexStringToBytes("12345678876543210000000000000000");
//    	byte[] key = Util.hexStringToBytes("31323334353637383837363534333231");
//    	byte[] key = new String("12345678876543210000000000000000").getBytes();
    	//方法一
    	String deString = SMS4.decodeSMS4toString(out, key);
    	//方法二
    	byte[] decode16 = SMS4.decodeSMS4(out, key);
    	//结果打印
    	System.out.println("方法一，解密后，密文长度：" + deString.length());
    	System.out.println("方法一，解密后，密文内容：" + deString);
    	String byteToString = Util.byteToHex(decode16);
    	System.out.println("方法二，解密后，密文16进制内容：" + byteToString);
    	String decode = Utils.decode(byteToString);
    	System.out.println("方法二，解密后转码，密文内容：" + decode);
	}
    
    public static String decodeTest4() {
    	byte[] in = Util.hexStringToBytes("313233342E3536200808080808080808");
    	byte[] key = Util.hexStringToBytes("12345678876543210000000000000000");
